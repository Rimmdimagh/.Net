...
{ 
    public class ServiceFlight : Service<Flight>,IServiceFlight
    {
        ...
        public IEnumerable<Flight> volOrdoParDateDepNDer(int n)
           {
            return Flights.OrderByDescending(flight => flight.FlightDate).Take(n);
        }
        public bool CanReserveSeats(Flight flight, int n)
        {
            return flight.Plane.Capacity - flight.Passengers.Count >= n;
        }
        public IList<Staff> GetStaffMembersByFlightId(int flightId)
        {
            Flight flight = GetById(flightId); 

            if (flight != null)
            {
                return flight.Passengers.OfType<Staff>().ToList();
            }

            return new List<Staff>(); 
        }

        public IList<Traveller> GetTravByPlaneAndDate(Plane pl, DateTime date)
        {
            return GetMany(p => p.FlightDate.Equals(date) && p.Plane.PlaneId==pl.PlaneId)
